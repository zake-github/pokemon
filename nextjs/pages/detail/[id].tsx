...
import { getDetail } from '../api/http';
import type {
  pokemonType,
  pokemonDetailType,
  pokemonTypeType
...
export default function Detail(props: any) {
  ...
  const len = detail && Object.keys(detail).length > 0;
                  ...
                  {!showAbility ? (
                    <div className='pokemon-info--inner'>
                      <div className='pokemon-info__height'>
                        <span className='pokemon-info__title size-14'>
                          身高
                        </span>
                ...
                <div className='pokemon-stats__wrapper-status'>
                  <div className='pokemon-status__block'>
                    ...
                    <div className='pokemon-status'>
                      <div className='pokemon-status__scale-box'>
                        ...
                        {Array.from({ length: detail.attack - 1 }).map(
                          (v, i) => (
                            <span
                              key={i}
                              className={
                  ...
                  <div className='pokemon-status__block'>
                    ...
                    <div className='pokemon-status'>
                      <div className='pokemon-status__scale-box'>
                        {Array.from({
                          length: 16 - detail.special_defense
                        }).map((v, i) => (
                          <span
                            className='pokemon-status__scale'
                            key={i}
                    ...
                    <div className='pokemon-status'>
                      <div className='pokemon-status__scale-box'>
                        {Array.from({ length: 16 - detail.speed }).map(
                          (v, i) => (
                            <span
                              className='pokemon-status__scale'
                              key={i}
                        ...
                        {Array.from({ length: detail.speed - 1 }).map(
                          (v, i) => (
                            <span
                              key={i}
                              className={
...
export async function getStaticPaths() {
  const maxId: any = await pokemonService.getMaxId();
  const res: any = await pokemonService.getPokemons({
    zukan_id_from: 1,
    zukan_id_to: maxId.data
  });
